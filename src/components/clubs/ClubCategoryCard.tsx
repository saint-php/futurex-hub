// src/components/clubs/ClubCategoryCard.tsx
import { useNavigate } from "react-router-dom";
import { ChevronRight } from "lucide-react";
import type { ClubCategory } from "../../data/clubCategories";

interface Props {
  category: ClubCategory & { numberOfIdeas: number };
}

export default function ClubCategoryCard({ category }: Props) {
  const navigate = useNavigate();

  return (
    <button
      type="button"
      onClick={() => navigate(`/dashboard/clubs/${category.id}`)}
    ...
    >
      ...
      <div className="relative flex items-start justify-between">
        ...
        <span className="rounded-full bg-white/80 px-2.5 py-0.5 text-[11px] font-bold text-slate-600">
          {category.numberOfIdeas}{" "}
          {category.numberOfIdeas === 1 ? "idea" : "ideas"}
        </span>
      </div>
